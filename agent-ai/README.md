...
Agent AI is a local-first PDF question-answering application based on the course's Express, React, RAG, voice, and Model Context Protocol lessons. It accepts a PDF, retrieves relevant passages for each question, and can optionally compare the grounded answer with live web results supplied by a child-process MCP server.
...
Uploaded text lives only in process memory and expires after `DOCUMENT_TTL_MS`. Restarting the server clears it immediately. This implementation intentionally avoids the tutorial's process-global `filePath`, unsafe original-name disk writes, and cross-user document leakage.
...
The response contains `ragAnswer`, page-aware `sources`, optional `mcpAnswer`, and optional `webSources`. Compatibility routes `/upload` and `/chat` are retained for the course's Postman exercises, though the versioned JSON API is preferred.
